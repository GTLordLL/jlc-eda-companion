...
## 六、生成 Design Spec（Phase 1→2 桥梁）

BOM 生成完成后，**应将手册知识转换为 Design Spec JSON**，供 Phase 2 ERC 引擎精确对照检查。

### 流程

```
BOM 生成完成 → 运行 design_spec.py init → 填充规范内容 → 运行 validate 校验
```
...
```bash
# 从模板初始化
python tools/design_spec.py init "51单片机最小系统板" --core-chip STC89C52RC --lcsc C8707 --package LQFP-44
...
详见 `prompts/design_spec_template.md` — 包含 7 个类别的逐项提取指南和完整示例。

### Design Spec 覆盖的 7 个类别

| 类别 | 说明 | 触发 Phase 2 检查 |
|------|------|------------------|
| `decoupling` | VCC/VDD 去耦电容值 | check_spec_decoupling |
| `pullup` | 上拉电阻值 | check_spec_pullup |
| `pulldown` | 下拉电阻值 | check_spec_pullup |
| `crystal` | 晶振负载电容 | check_spec_crystal |
| `power_feedback` | 电源反馈分压 | check_spec_power |
| `pin_termination` | 引脚端接要求 | check_spec_pin_config |
| `pin_exclusion` | 排除误报引脚 | 各检查排除列表 |

### 核心原则

1. **数值双写**：字符串 + SI 浮点（`"100nF"` + `1e-7`）
2. **source 必须精确**：每条 requirement 必须包含手册章节号
3. **不确定标注置信度**：如手册信息不完整，标注 `"confidence": "low"`


## 七、与后续阶段的衔接

方案阶段输出的 BOM 和 Design Spec，将直接用于：

- **Design Spec（桥梁）**：结构化手册知识 → Phase 2 ERC 精确对照
- **阶段二 ERC**：`phase2_erc_check.py --spec design_spec.json` 检查原理图是否与手册一致
